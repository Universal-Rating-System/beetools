...
class Archiver:
    ...
    def __init__(
        ...
        # self.app_proj_fldr = self.app_path.parents[2]
        self.app_ver = p_app_ver
        # self.app_ver_fldr = self.app_path.parents[ 1 ]
        ...
        self.elapsed_time = 0
        self.end_time = 0
        # self.proj_fldr = self.app_path.parents[2]
...
def example_virtual_environment():
    ...
    success = (
        set_up_venv(get_tmp_fldr(), venv_name, package_list, p_verbose=True)
        and success
    )
...
def example_messaging():
    ...
    print(
        msg_display(
            'This message print in blue and cut at {} character because it is too long!'.format(MSG_LEN)
        )
    )
